...
Citizen.CreateThread(function()
	...
	while VRP.GetPlayerData().job == nil do
		Citizen.Wait(10)
	end
...
AddEventHandler('vrp:playerLoaded', function(xPlayer)
	VRP.PlayerData = xPlayer
end)

RegisterNetEvent('vrp:setJob')
AddEventHandler('vrp:setJob', function(job)
	VRP.PlayerData.job = job
	isPlayerWhitelisted = refreshPlayerWhitelisted()
end)

RegisterNetEvent('vrp_TruckRobbery:outlawNotify')
AddEventHandler('vrp_TruckRobbery:outlawNotify', function(alert)
...
function refreshPlayerWhitelisted()	
	if not VRP.PlayerData then
		return false
	end

	if not VRP.PlayerData.job then
		return false
	end

	if Config.PoliceDatabaseName == VRP.PlayerData.job.name then
		return true
	end
		...
		for k,v in pairs(Config.MissionSpot) do
			...
			if distance <= 1.0 then
				...
				if IsControlJustPressed(0, Config.KeyToStartMission) then
					TriggerServerEvent("vrp_TruckRobbery:missionAccepted")
					Citizen.Wait(500)
				end
			end
		end		
...
RegisterNetEvent('vrp_TruckRobbery:TruckRobberyInProgress')
AddEventHandler('vrp_TruckRobbery:TruckRobberyInProgress', function(targetCoords)
...
RegisterNetEvent("vrp_TruckRobbery:HackingMiniGame")
AddEventHandler("vrp_TruckRobbery:HackingMiniGame",function()
	toggleHackGame()
end)
...
function AtmHackSuccess(success)
    ...
    if success then
		VRP.TriggerServerCallback("vrp_TruckRobbery:StartMissionNow",function()	end)
    else
		VRP.ShowNotification(Config.HackingFailed)
...
RegisterNetEvent("vrp_TruckRobbery:startMission")
AddEventHandler("vrp_TruckRobbery:startMission",function(spot)
	...
	if numy == 100 then
		VRP.ShowNotification(Config.NoMissionsAvailable)
	else
		CurrentEventNum = num
		TriggerEvent("vrp_TruckRobbery:startTheEvent",num)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		VRP.ShowNotification(Config.TruckMarkedOnMap)
	end
end)

-- Core Mission Part
RegisterNetEvent('vrp_TruckRobbery:startTheEvent')
AddEventHandler('vrp_TruckRobbery:startTheEvent', function(num)
	
	local loc = Config.ArmoredTruck[num]
	Config.ArmoredTruck[num].InUse = true
	local playerped = GetPlayerPed(-1)
	
	TriggerServerEvent("vrp_TruckRobbery:syncMissionData",Config.ArmoredTruck)
	...
	while not taken do
		...
		if missionInProgress == true then
			...
			if distance <= 30.0  then
				if KillGuardsText == false then
					VRP.ShowNotification(Config.KillTheGuards)
					KillGuardsText = true
				end
			end
			
			if distance <= 5 and TruckIsDemolished == false then
				VRP.ShowHelpNotification(Config.OpenTruckDoor)
		...
		if TruckIsExploded == true then
			...
			if distance <= 4.5 then
				VRP.ShowHelpNotification(Config.RobFromTruck)
		...
		if missionCompleted == true then
			VRP.ShowNotification(Config.MissionCompleted)
	...
	if IsVehicleStopped(ArmoredTruckVeh) then
		if (IsVehicleSeatFree(ArmoredTruckVeh, -1) and IsVehicleSeatFree(ArmoredTruckVeh, 0) and IsVehicleSeatFree(ArmoredTruckVeh, 1)) then
			...
			if Config.PoliceNotfiyEnabled == true then
				TriggerServerEvent('vrp_TruckRobbery:TruckRobberyInProgress',GetEntityCoords(PlayerPedId()),streetName)
			end
			...
			VRP.ShowNotification(Config.BeginToRobTruck)
		else
			VRP.ShowNotification(Config.GuardsNotKilledYet)
		end
	else
		VRP.ShowNotification(Config.TruckIsNotStopped)
	end
end
...
function RobbingTheMoney()
	...
	TriggerServerEvent("vrp_TruckRobbery:missionComplete")
...
-- Sync mission data
RegisterNetEvent("vrp_TruckRobbery:syncMissionData")
